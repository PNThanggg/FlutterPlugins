...
class MethodChannelBattery extends BatteryPlatform {
  ...
  @visibleForTesting
  MethodChannel methodChannel = const MethodChannel('com.example.flutter_plugins/battery');

  /// The event channel used to receive BatteryState changes from the native platform.
  @visibleForTesting
  EventChannel eventChannel = const EventChannel('com.example.flutter_plugins/charging');
  ...
  Future<int> get batteryLevel =>
      methodChannel.invokeMethod<int>('getBatteryLevel').then<int>((dynamic result) => result);
