...
class Toast {
  ...
  static const MethodChannel _channel = MethodChannel('com.example.flutter_plugins/toast');
...
class FToast {
  ...
  void showToast({
    ...
    _overlayQueue
        .add(_ToastEntry(entry: newEntry, duration: toastDuration, fadeDuration: fadeDuration));
  ...
  _getPostionWidgetBasedOnGravity(Widget child, ToastGravity? gravity) {
    switch (gravity) {
      ...
      case ToastGravity.SNACKBAR:
        return Positioned(
            bottom: MediaQuery.of(context!).viewInsets.bottom, left: 0, right: 0, child: child);
...
class _ToastStateFul extends StatefulWidget {
  _ToastStateFul(this.child, this.duration, this.fadeDuration, {Key? key}) : super(key: key);
